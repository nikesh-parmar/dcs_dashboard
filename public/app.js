const els = {
  ...
  supportContactsBody: document.getElementById("supportContactsBody"),
  supportContactsNote: document.getElementById("supportContactsNote"),
  supportContactsFooter: document.getElementById("supportContactsFooter"),
...
function renderAudit(data) {
  ...
  renderSupportContacts(data.accountContacts || null);
  renderHubHome(data);
...
function renderPersonCell(person) {
  if (!person) return "—";
  const avatarClass = person.isSupport ? "support-avatar support-avatar-brand" : "support-avatar";
  const nameHtml = person.url
    ? `<a href="${escapeHtml(person.url)}" target="_blank" rel="noopener noreferrer">${escapeHtml(
        person.name
      )}</a>`
    : escapeHtml(person.name);
  return `
    <div class="support-person">
      <span class="${avatarClass}" aria-hidden="true">${escapeHtml(person.initials || "?")}</span>
      <div>
        <div class="support-person-name">${nameHtml}</div>
        <div class="muted support-person-role">${escapeHtml(person.role || person.title || "")}</div>
      </div>
    </div>`;
}

function getDefaultSupportContacts(project = {}) {
  const account = String(project.name || project.workspace || "this account").trim() || "this account";
  return {
    demo: true,
    accountName: account,
...
function renderSupportContacts(accountContacts) {
  const contacts = accountContacts || getDefaultSupportContacts(auditData?.project || {});
  if (els.supportContactsNote) {
    els.supportContactsNote.textContent = contacts.note || "Request types by person, with escalation points.";
  }
  if (els.supportContactsFooter) {
    els.supportContactsFooter.textContent =
      contacts.footer || "If an issue is unresolved or urgent, escalate through your CSM first.";
  }
  const body = els.supportContactsBody;
  if (!body) return;
  const rows = Array.isArray(contacts.rows) ? contacts.rows : [];
  if (!rows.length) {
    body.innerHTML = `<tr><td colspan="3" class="muted">No account contacts available.</td></tr>`;
    return;
  }
  body.innerHTML = rows
    .map(
    ...
    <tr>
      <td>
        <div class="support-enquiry">${escapeHtml(row.enquiry)}</div>
        <div class="muted support-enquiry-detail">${escapeHtml(row.detail || "")}</div>
      </td>
      <td>${renderPersonCell(row.firstContact)}</td>
      <td>${renderPersonCell(row.escalation)}</td>
    </tr>`
    )
    .join("");
}
